<script setup>
import { SquarePlus, ListVideo, ListCheck, ClockArrowDown, Logs, Search, Plus, X } from 'lucide-vue-next';
</script>
<template>
        ...
        <div class="mt-8 flex-grow flex">
            <div class="tache-late border border-gray-300 rounded-lg p-4 w-full flex flex-col">
                <div class="title-div1 flex justify-between items-center border-b border-gray-300 pb-2 mb-4">
                    ...
                    <button v-if="true" class="flex gap-1 bg-black hover:hover:bg-gray-600 text-white px-3 py-2 rounded-lg"
                        @click="addtask()">
                        <Plus class="h-6 " />
    ...
    <!-- Ici, tu montes le composant enfant et tu écoutes l'événement `create-task` -->
    <tasks-page @create-task="createNewTask" />
</template>
...
export default {
    ...
    methods: {
        ...
        async createNewTask() {
            ...
            // Fonction pour s'assurer que la date est au bon format ISO
            const formatDateForPrisma = (dateStr) => {
                const date = new Date(dateStr);
                return date.toISOString(); // Prisma attend ce format ISO 8601
           };
            ...
            if (this.userAssignId === "" || this.userAssignId === null) {
                ...
                try {
                    ...
                    const response = await axios.post(`${config.apiBaseUrl}/tasks`, {
                        ...
                        start_date: formatDateForPrisma(this.start_date), // ✅ conversion correcte
                        end_date: formatDateForPrisma(this.end_date),
