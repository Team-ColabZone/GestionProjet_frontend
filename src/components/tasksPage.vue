    ...
    <p class="text-lg">Tâches</p>
        ...
        <div class="flex flex-col sm:flex-row justify-between">
            ...
            <div class="filter mt-4 sm:mt-0 sm:ml-4">
                ...
                <div :class="{ block: task.showMenu, hidden: !task.showMenu }" v-if="task && task.showMenu"
...
export default defineComponent({
  ...
  // Exemple pour récupérer l'ID utilisateur depuis le token
    const tokenPayload = JSON.parse(atob(localStorage.getItem('token').split('.')[1]));
    this.userId = tokenPayload.sub; // ou tokenPayload.userId selon ton backend
}
    },
    methods: {
        refreshAllData() {
